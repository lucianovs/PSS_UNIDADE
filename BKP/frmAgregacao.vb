...
Public Class frmAgregacao
    ...
    Private Sub TratarObjetos()
        Dim nCodUnidade As Integer
        Dim sNomeUnidade As String
        Dim sClaUni As String
        Dim nCodUni As Integer
        ...
        lblClaUni.Enabled = False Or (bAlterar And UCase(ClassCrypt.Decrypt(g_Login)) = "ADMIN")
        txtClaUni.Enabled = False Or (bAlterar And UCase(ClassCrypt.Decrypt(g_Login)) = "ADMIN")
        lblID_CF.Enabled = False Or (bAlterar And UCase(ClassCrypt.Decrypt(g_Login)) = "ADMIN")
        txtID_CF.Enabled = False Or (bAlterar And UCase(ClassCrypt.Decrypt(g_Login)) = "ADMIN")
        ...
        cbCP.Enabled = bAlterar And bEmDigitacao
        lblCC.Enabled = False 'bAlterar
        cbCC.Enabled = False 'bAlterar
        lblCM.Enabled = False 'bAlterar
        cbCM.Enabled = False 'bAlterar
        ...
        If i > -1 And Not bIncluir Then
            ...
            'If Not IsDBNull(dt.Rows(i).Item("UN015_CODCM")) Then
            'If dt.Rows(i).Item("UN015_CODCM") = 0 Then
            'sClaUni = Microsoft.VisualBasic.Left(txtClaUni.Text, 2) & ".00.00.00"
            'nCodUni = LerCod_Unidade(sClaUni, sNomeUnidade)
            'Else
            'sClaUni = LerClasse_Unidade(dt.Rows(i).Item("UN015_CODCM"), sNomeUnidade)
            'End If
            'cbCM.Text = sClaUni & " - " & sNomeUnidade
            'Else
            'cbCM.Text = ""
            'End If

            'If Not IsDBNull(dt.Rows(i).Item("UN015_CODCC")) Then
            'If dt.Rows(i).Item("UN015_CODCC") = 0 Then
            'sClaUni = Microsoft.VisualBasic.Left(txtClaUni.Text, 5) & ".00.00"
            'nCodUni = LerCod_Unidade(sClaUni, sNomeUnidade)
            'Else
            'sClaUni = LerClasse_Unidade(dt.Rows(i).Item("UN015_CODCC"), sNomeUnidade)
            'End If
            'cbCC.Text = sClaUni & " - " & sNomeUnidade
            'Else
            '    cbCC.Text = ""
            'End If

            If Not IsDBNull(dt.Rows(i).Item("UN015_CODCP")) Then
                If dt.Rows(i).Item("UN015_CODCP") = 0 Then
                    sClaUni = Microsoft.VisualBasic.Left(txtClaUni.Text, 8) & ".00"
                    nCodUni = LerCod_Unidade(sClaUni, sNomeUnidade)
                Else
                    sClaUni = LerClasse_Unidade(dt.Rows(i).Item("UN015_CODCP"), sNomeUnidade)
                End If
                cbCP.Text = sClaUni & " - " & sNomeUnidade
            Else
                cbCP.Text = ""
            End If
        ...
        If bIncluir Then
            'Carregar o Combo de Unidades (CPs)
            cQueryTemp = "SELECT EUN000.UN000_CLAUNI, EUN000.UN000_NOMUNI "
            If nCodUsuario.ToString <> 1 Then
                cQueryTemp += "FROM (EUN000 inner join EUN013 on EUN013.UN013_CODUNI=EUN000.UN000_CODRED) "
                cQueryTemp += "WHERE EUN000.UN000_STAUNI='A' AND RIGHT(UN000_CLAUNI,2)='00' AND RIGHT(UN000_CLAUNI,5)<>'00.00' "
                cQueryTemp += "AND EUN013.UN013_PERACE > 1 "
                cQueryTemp += "AND EUN013.UN013_CODUSU = " & nCodUsuario.ToString
            Else
                cQueryTemp += "FROM EUN000 "
                cQueryTemp += "WHERE EUN000.UN000_STAUNI='A' AND RIGHT(UN000_CLAUNI,2)='00' AND RIGHT(UN000_CLAUNI,5)<>'00.00' "
            End If
            cQueryTemp += " ORDER BY EUN000.UN000_CLAUNI"

            Using da As New OleDbDataAdapter()
                da.SelectCommand = New OleDbCommand(cQueryTemp, g_ConnectBanco)

                'Preencher o DataTable 
                da.Fill(dtUnidade)
                If dtUnidade.Rows.Count() > 0 Then
                    For x = 0 To dtUnidade.Rows.Count() - 1
                        cbCP.Items.Add(dtUnidade.Rows(x).Item("UN000_CLAUNI") & " - " & dtUnidade.Rows(x).Item("UN000_NOMUNI"))
                    Next
                Else
                    cbCP.Items.Clear()
                End If
        ...
        ElseIf bAlterar Then
            'Ler as Unidades da Agregação
            sNomeUnidade = ""
            'CM
            nCodUnidade = LerCod_Unidade(Microsoft.VisualBasic.Left(txtClaUni.Text, 2) & ".00.00.00", sNomeUnidade)
            cbCM.Text = Microsoft.VisualBasic.Left(txtClaUni.Text, 2) & ".00.00.00" & " - " & sNomeUnidade
            'CC
            nCodUnidade = LerCod_Unidade(Microsoft.VisualBasic.Left(txtClaUni.Text, 5) & ".00.00", sNomeUnidade)
            cbCC.Text = Microsoft.VisualBasic.Left(txtClaUni.Text, 5) & ".00.00" & " - " & sNomeUnidade
            'CP
            nCodUnidade = LerCod_Unidade(Microsoft.VisualBasic.Left(txtClaUni.Text, 8) & ".00", sNomeUnidade)
            cbCP.Text = Microsoft.VisualBasic.Left(txtClaUni.Text, 8) & ".00" & " - " & sNomeUnidade
        End If
    ...
    Private Sub btnGravar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGravar.Click
        ...
        If ConectarBanco() Then
            ...
            If bIncluir Then
                ...
                cSql += ",'" & cbClaCfr.Text & "','" & txtPaiCFR.Text & "','" & cbEstCFR.Text & "'," & LerCod_Unidade(Microsoft.VisualBasic.Left(txtClaUni.Text, 3) & "00.00.00").ToString
                cSql += "," & LerCod_Unidade(Microsoft.VisualBasic.Left(txtClaUni.Text, 6) & "00.00").ToString
                cSql += "," & LerCod_Unidade(Microsoft.VisualBasic.Left(txtClaUni.Text, 9) & "00").ToString & ",0"
    ...
    Private Sub frmAgregacao_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        'Montar o Combo do Cons.Particular
        'Carregar os CPs

        cbCP.Items.Clear()
        'Carregar o Combo de Unidades
        cQueryTemp = "SELECT UN000_CLAUNI, UN000_NOMUNI "
        If LCase(ClassCrypt.Decrypt(g_Login)) = "admin" Then
            cQueryTemp += "FROM EUN000 where "
        Else
            cQueryTemp += "FROM (EUN000 inner join EUN013 on EUN013.UN013_CODUNI=EUN000.UN000_CODRED)"
            cQueryTemp += " where EUN013.UN013_CODUSU=" & nCodUsuario.ToString & _
                "AND UN013_PERACE = 3 and "
        End If
        cQueryTemp += " RIGHT(UN000_CLAUNI,3)='.00' order by UN000_CLAUNI"
        Using da As New OleDbDataAdapter()
            da.SelectCommand = New OleDbCommand(cQueryTemp, g_ConnectBanco)
    ...
    Private Sub cbCM_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cbCM.SelectedIndexChanged
        ...
        If bIncluir Then
            ...
            cbCC.Items.Clear()
            'Carregar o Combo de Unidades
            cQueryTemp = "SELECT UN000_CLAUNI, UN000_NOMUNI FROM EUN000 " & _
                "WHERE LEFT(UN000_CLAUNI,2)='" & Microsoft.VisualBasic.Left(cbCM.Text, 2) & "' AND " & _
                "RIGHT(UN000_CLAUNI,6)='.00.00'"
            Using da As New OleDbDataAdapter()
                da.SelectCommand = New OleDbCommand(cQueryTemp, g_ConnectBanco)
                ...
                If dtUnidade.Rows.Count() > 0 Then
                    For x = 0 To dtUnidade.Rows.Count() - 1
                        cbCC.Items.Add(dtUnidade.Rows(x).Item("UN000_CLAUNI") & " - " & dtUnidade.Rows(x).Item("UN000_NOMUNI"))
                    Next
                End If
                cbCP.Items.Clear()
            End Using
    ...
    Private Sub cbCP_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cbCP.SelectedIndexChanged
        Dim nCodigo As Integer
        Dim sClaUni As String
        Dim sNomeUnidade As String = ""

        'CM
        sClaUni = Microsoft.VisualBasic.Left(cbCP.Text, 2) & ".00.00.00"
        nCodigo = LerCod_Unidade(sClaUni, sNomeUnidade)
        If nCodigo > 0 Then
            cbCM.Text = sClaUni & " - " & sNomeUnidade
        Else
            cbCM.Text = ""
        ...
        'CC
        sClaUni = Microsoft.VisualBasic.Left(cbCP.Text, 5) & ".00.00"
        nCodigo = LerCod_Unidade(sClaUni, sNomeUnidade)
        If nCodigo > 0 Then
            cbCC.Text = sClaUni & " - " & sNomeUnidade
        Else
    ...
    Private Sub cbCC_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cbCC.SelectedIndexChanged
        ...
        If bIncluir Then
            ...
            'Carregar o Combo de Unidades
            cQueryTemp = "SELECT UN000_CLAUNI, UN000_NOMUNI FROM EUN000 " & _
                "WHERE LEFT(UN000_CLAUNI,5)='" & Microsoft.VisualBasic.Left(cbCC.Text, 5) & "' AND " & _
                "RIGHT(UN000_CLAUNI,3)='.00'"
            Using da As New OleDbDataAdapter()
                da.SelectCommand = New OleDbCommand(cQueryTemp, g_ConnectBanco)
    ...
    Private Sub MostrarClasse()

        txtClaUni.Text = Microsoft.VisualBasic.Left(cbCM.Text, 2) & "."
        If cbCC.Text <> "" Then
            txtClaUni.Text += Microsoft.VisualBasic.Mid(cbCC.Text, 4, 2) & "."
        Else
            txtClaUni.Text += "00."
        End If
        If cbCP.Text <> "" Then
            txtClaUni.Text += Microsoft.VisualBasic.Mid(cbCP.Text, 7, 2) & "."
        Else
            txtClaUni.Text += "00."
        End If
        If Not txtID_CF.Text = "" Then
            If CInt(txtID_CF.Text) = 0 Then txtClaUni.Text += "00"
        Else
            txtClaUni.Text += "00"
        End If
        txtClaUni.Text = ProxSeq_Unidade(txtClaUni.Text)
    End Sub
    ...
    Private Sub CarregarGrid(fCodMandato As Integer)
        ...
        cQuery = "Select EUN012.UN012_CODOCP as Encargo, EUN011.UN011_DESOCP as Descricao, " & _
            "EUN012.UN012_CODCOL as Cod_Colab, EUN003.UN003_NOMCOL as Nome_Colab " & _
        ...
        If dtGrid.Rows.Count = 0 Then
            ...
            If dtGrid.Rows.Count > 0 Then
                For x = 0 To dtGrid.Rows.Count - 1
                    'INSERIR O MANDATO
                    cQuery = "INSERT INTO EUN012 (UN012_CODMDT, UN012_CODOCP, UN012_CODCOL) VALUES " & _
                        "(" & fCodMandato & "," & dtGrid.Rows(x).Item("UN011_CODOCP") & ",0)"
    ...
    Private Function InserirMandato() As Boolean
        Dim cmdMandato As OleDbCommand

        cQuery = "INSERT INTO EUN016 (UN016_CODMDT, UN016_CODRED, UN016_DESMDT, UN016_DATINI, UN016_DATFIN) VALUES " & _
        "(" & ProxCodChave("EUN016", "UN016_CODRED,UN016_CODMDT", txtID_CF.Text) & _
        "," & txtID_CF.Text & ",'MANDATO " & Year(Today).ToString & "','" & _
    ...
    Private Sub ComandoPesquisar_Click(sender As Object, e As EventArgs) Handles ComandoPesquisar.Click
        ...
        If Not Trim(NomePesquisar.Text) = "" Then
            cQuery = "Select EUN003.UN003_CODCOL, EUN003.UN003_NOMCOL from EUN003 " & _
            ...
            For nSeq = 0 To dtPesquisar.Rows.Count - 1
                ListaPesquisa.Items.Add(Format(dtPesquisar.Rows(nSeq).Item("UN003_CODCOL"), "000000") & " | " & dtPesquisar.Rows(nSeq).Item("UN003_NOMCOL"))
            Next nSeq
